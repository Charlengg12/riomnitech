...
function ProductsPage() {
  ...
  const priceCeiling = useMemo(
    () => products.length ? Math.ceil(Math.max(...products.map((p) => p.price)) / 50) * 50 : 500,
    [products],
  );
  ...
  }, [products, q, category, inStock, effectiveMax, sort]);
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-14 lg:px-8 lg:py-20">
      <header className="max-w-2xl">
        ...
        <p className="mt-3 text-muted-foreground">Hand-picked robotics, electronics and tooling — curated by engineers.</p>
      </header>
        ...
        <aside className="space-y-8 lg:sticky lg:top-24 lg:self-start">
          <div>
            ...
            <div className="relative mt-2">
              ...
              <Input id="search" type="search" value={q} onChange={(e) => update({ q: e.target.value })} placeholder="Search products, specs, SKU…" className="pl-9" />
            </div>
          </div>
          <div>
                ...
                return (
                  <li key={c}>
                    <button
                      ...
                      className={`flex w-full items-center justify-between rounded-md px-3 py-1.5 text-sm transition-colors ${isActive ? "bg-foreground text-background" : "text-muted-foreground hover:bg-secondary hover:text-foreground"}`}
                    >
                      <span>{c}</span>
                      <span className={`text-xs ${isActive ? "text-background/70" : "text-muted-foreground/70"}`}>{counts.get(c) ?? 0}</span>
                    </button>
                  </li>
                );
              })}
          ...
          <div>
            ...
            <label className="mt-3 flex cursor-pointer items-center gap-2 text-sm text-muted-foreground">
              <Checkbox checked={inStock} onCheckedChange={(v) => update({ inStock: v === true })} />
              In stock only
            </label>
          </div>
          <div>
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm tabular-nums text-muted-foreground">${effectiveMax}</span>
            </div>
            <input type="range" min={0} max={priceCeiling} step={10} value={effectiveMax}
              onChange={(e) => update({ maxPrice: Number(e.target.value) })} className="mt-3 w-full accent-primary" />
            <div className="mt-1 flex justify-between text-xs text-muted-foreground"><span>$0</span><span>${priceCeiling}</span></div>
          </div>
          {hasActiveFilters && (
        ...
        <div>
          {activeChips.length > 0 && (
            <div className="mb-4 flex flex-wrap items-center gap-2">
              ...
              <button onClick={resetAll} className="ml-1 text-xs font-medium text-primary underline-offset-4 hover:underline">Clear all</button>
            </div>
          )}
          ...
          {loading ? (
            ...
            <div className="mt-16 rounded-lg border border-dashed border-border py-20 text-center">
              ...
              <Button variant="outline" className="mt-6" onClick={resetAll}>Reset filters</Button>
            </div>
          ) : (
